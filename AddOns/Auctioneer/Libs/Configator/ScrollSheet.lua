...
function kit:Render()
	...
	for i = 1, #rows do
		...
		if colorTex.SetGradientAlpha then -- ### hybrid
			colorTex:SetGradientAlpha(direction, rowR, rowG, rowB, rowA1, rowR, rowG, rowB, rowA2)--row color to apply
		else
...
function lib:Create(frame, layout, onEnter, onLeave, onClick, onResize, onSelect)
	...
	for i = 1, #layout do
		...
		if button.SetResizeBounds then -- detect which bounds method to use -- ### hybrid
			button:SetResizeBounds(13, 16, 400, 16)
		else
